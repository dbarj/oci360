...
from   OCI360_USAGECOSTS_TAGGED_HOURLY;
...
SELECT 'DEF title = ''Tag: ' || TAG || '''' || CHR(10) ||
       ...
       'DEF main_table = ''OCI360_USAGECOSTS_TAGGED_HOURLY''' || CHR(10) ||
       'DEF vaxis = ''Cost (&&oci360_billing_currency.)''' || CHR(10) ||
       'EXEC :sql_text := REPLACE(:sql_text_backup, ''@main_table@'', q''[OCI360_USAGECOSTS_TAGGED_HOURLY]'');' || CHR(10) ||
...
FROM   ( SELECT   DISTINCT t1.TAG
         FROM     OCI360_USAGECOSTS_TAGGED_HOURLY t1
         WHERE EXISTS (SELECT 1 FROM OCI360_USAGECOSTS_TAGGED_HOURLY t3 WHERE t3.TAG = t1.TAG HAVING SUM(COSTS$COMPUTEDAMOUNT) > 0)
...
DEF title = 'Top 15 Resources - Total per hour'
DEF title_suffix = '&&oci360_billing_between.'
DEF main_table = 'OCI360_USAGECOSTS'
DEF vaxis = 'Cost (&&oci360_billing_currency.)'
DEF chartype = 'AreaChart'
