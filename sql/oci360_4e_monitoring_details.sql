-----------------------------------------
DEF oci360_monit_sec_start = '&&moat369_sw_folder./&&moat369_sw_name._&&moat369_sec_id._monit_section_start.sql'
DEF oci360_monit_sec_stop  = '&&moat369_sw_folder./&&moat369_sw_name._&&moat369_sec_id._monit_section_stop.sql'
...
DEF oci360_monit_between = ', between &&oci360_monit_date_from. and &&oci360_monit_date_to.'
...
@@&&fc_def_output_file. oci360_loop_section 'oci360_4e_vnic_section.sql'
@@&&fc_spool_start.
...
@@&&fc_def_output_file. oci360_loop_section 'oci360_4e_cpu_section.sql'
@@&&fc_spool_start.
SPO &&oci360_loop_section.
SELECT '----------------------' || CHR(10) ||
       'DEF title = ''' || COMPUTE_DESC || '''' || CHR(10) ||
       'DEF title_suffix = ''&&oci360_monit_between.''' || CHR(10) ||
       'DEF main_table = ''OCI360_MONIT_METRIC_DATA_1HMAX''' || CHR(10) ||
       'DEF foot = ''Vnic: ' || COMPUTE_ID || '''' || CHR(10) ||
       'DEF vaxis = ''% CPU''' || CHR(10) ||
       '-- Dont remove this TRIM or sql_text_base may be removed.' || CHR(10) ||
       'EXEC :sql_text := TRIM(:sql_text_base);' || CHR(10) ||
       '--' || CHR(10) ||
       'EXEC :sql_text := REPLACE(:sql_text, ''@filter_predicate@'', q''[DIMENSIONS$RESOURCEID = ''' || COMPUTE_ID || ''' AND NAMESPACE = ''oci_computeagent'' AND NAME = ''CpuUtilization'']'');' || CHR(10) ||
       '-- Removing lines with comments.' || CHR(10) ||
       'EXEC :sql_text := REGEXP_REPLACE(:sql_text, ''--.*'' || CHR(10), '''');' || CHR(10) ||
       '--' || CHR(10) ||
       'DEF tit_01 = ''CPU Utilization''' || CHR(10) ||
       'EXEC :sql_text := REPLACE(:sql_text, ''@line_01@'', ''CpuUtilization'');' || CHR(10) ||
       '--' || CHR(10) ||
       'DEF chartype = ''LineChart''' || CHR(10) ||
       'DEF skip_lch = ''''' || CHR(10) ||
       '@@&&9a_pre_one.'
FROM   ( SELECT DISTINCT t1.DIMENSIONS$RESOURCEID COMPUTE_ID,
                DECODE(t2.id,NULL,t1.DIMENSIONS$RESOURCEID,
                    DECODE(t2.display_name,NULL,NULL,'Name: ' || t2.display_name || ' ') ||
                    DECODE(t2.display_name,NULL,NULL,'Region: ' || t2.region || ' ')) COMPUTE_DESC
         FROM   OCI360_MONIT_METRIC_DATA_1HMAX t1, OCI360_INSTANCES t2
         WHERE  t1.NAMESPACE = 'oci_computeagent'
           AND  t1.DIMENSIONS$RESOURCEID = t2.id (+)
         ORDER BY 2);
SPO OFF
@@&&fc_spool_end.
