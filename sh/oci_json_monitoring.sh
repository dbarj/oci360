...
#************************************************************************
#
#   oci_json_monitoring.sh - Export all Oracle Cloud Audit monitoring
...
## https://docs.cloud.oracle.com/iaas/Content/Monitoring/Concepts/monitoringoverview.htm#MetricsOverview
## https://docs.cloud.oracle.com/iaas/api/#/en/monitoring/20180401/MetricData
...
function jsonMetricData ()
{
  ##########
  # This function will get your monitoring info desired range and split it in chunks of 3 hours.
  ...
  if [ ${v_start_epoch} -lt $((${v_today_epoch}-${v_storage_limit})) ]
  then
    echoDebug "Moving start time from $(ConvEpochToYMDhms ${v_start_epoch}) to $(ConvEpochToYMDhms $((${v_today_epoch}-${v_storage_limit}))) for \"${v_arg1}\" frames."
    v_start_epoch=$((${v_today_epoch}-${v_storage_limit}))
  fi
  ...
  v_oci_monit_qry="monitoring metric-data summarize-metrics-data"

  while true
  do
    v_search="${v_oci_monit_qry} ${v_next_date_start} ${v_next_date_end}Z ${v_arg1} ${v_arg2} ALL_ITEMS"
    ...
    then
      # Run
      set +e
      v_out=$(jsonAllMetricList "${v_oci_monit_qry} --start-time "${v_next_date_start}Z" --end-time "${v_next_date_end}Z"" "${v_jq_filter}" "${v_arg1}" "${v_arg2}")
      v_ret=$?
      set -e
...
function jsonAllMetricList ()
{
  ...
  set -eo pipefail # Exit if error in any call.
  set ${v_dbgflag} # Enable Debug
  [ "$#" -eq 4 -a "$1" != ""  -a "$2" != "" -a "$3" != "" -a "$4" != "" ] || { echoError "${FUNCNAME[0]} needs 2 parameters"; return 1; }
  local v_arg1 v_arg2 v_out v_fout l_items v_item v_ret v_ret_final
  local v_vl1 v_vl2 v_vl3
  v_arg1="$1" # Main oci call
  v_arg2="$2" # JQ Filter clause
  v_arg3="$3" # Resolution. Supported values: 1m-60m (also 1h).
  v_arg4="$4" # Aggregated Datapoints. max, mean, min, p50, p90, ...

  v_ret_final=0
  l_items=$(Monit-MetricsList | ${v_jq} -rc '.data[] | [."compartment-id",."namespace",."name"]' | sort -u)

  IFS=$'\n'
  for v_item in $l_items
  do
    IFS=' '
    v_vl1=$(${v_jq} -r '.[0]' <<< "${v_item}")
